  ...
  describe 'POST /oauth/token' do
    describe 'grant_type password' do
      context 'with valid params' do
        let(:user) do
          create :user, email: 'user@example.com', password: '12345678'
        end

        context 'without client credentials' do
          it 'returns token',
             reqres_title: 'Get access token without client credentials' do
        ...
        context 'with client credentials' do
          ...
          it 'returns token',
             reqres_title: 'Get access token with client credentials' do
    ...
    describe 'grant_type refresh_token' do
      ...
      let(:refresh_token) do
        client_application.access_tokens.create!(
          use_refresh_token: true, resource_owner_id: user.id
        ).refresh_token
      end
      ...
      it 'returns new refresh_token', reqres_title: 'Get new refresh token' do
        ...
        expect(
          Doorkeeper::AccessToken.second.application_id
        ).to eq client_application.id
