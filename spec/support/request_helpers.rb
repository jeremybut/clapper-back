...
module Requests
  module JsonHelpers
    ...
    def method_missing(method, *args)
      ...
      if args.first.blank?
        raise "#{method} expects a valid argument,
        '#{args.first}' passed"
      end
      raise "#{args.first.class} is not persisted" if args.first.try(:id).blank?

      method.to_s.gsub(/_helper$/, 'Serializer')
            .classify.constantize
            .new(args.first, root: nil).attributes.as_json
    end
  end
  ...
  module AuthenticationHelpers
    ...
    [:post, :put, :delete, :patch].each do |m|
      define_method "#{m}_as_user" do |url, opts = {}|
        opts = {} if opts.blank?
        opts = opts.to_json unless opts.values.any? do |v|
          v.class == Rack::Test::UploadedFile
        end
    ...
    def _user
      unless respond_to?(:user) || user.present?
        raise '"user" variable in request
        tests is not defined'
      end
      user
    end

    def _access_token
      @_access_token ||= create(
        :access_token,
        application: _client_application,
        resource_owner_id: _user.id
      ).token
    end

    def _client_application
      @_client_application ||= try(:client_application) ||
